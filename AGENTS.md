...
- **caddy/** — Reverse proxy (Caddy) with TLS via Porkbun DNS. Owns the `caddy-default` network.
- **kener/** — Status page (Kener) with Redis sidecar. Connects to `caddy-default`.
- **pocket-id/** — OIDC identity provider (Pocket-ID). Connects to `caddy-default`.
- **pairdrop/** — Local file sharing (PairDrop). Connects to `caddy-default`.
...
- Services needing reverse proxy access join the external `caddy-default` network.
